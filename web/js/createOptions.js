...
export function createOptions(type, data, id, zoneId) {
  ...
  const label = data.label.charAt(0).toUpperCase() + data.label.slice(1).toLowerCase()
  const iconElement = `<i class="fa-fw ${data.icon} option-icon" ${
    data.iconColor ? `style = color:${data.iconColor} !important` : null
  }"></i>`;

  option.innerHTML = `${iconElement}<p class="option-label">${label}</p>`;
  ...
  if($(".option-container").length > 1){
      $("#options-wrapper").css("justify-content", "space-between");
  }else{
    $("#options-wrapper").css("justify-content", "flex-end");
  }
}
